...
class App:
    def __init__(self):
        ...
        self.sql_delete_script = f'{self.work_dir}/delete-horaire-train.sql'
    ...
    def fn_delete_db(self):
        ...
        try:            
            with sqlite3.connect(self.db_name, timeout=10) as sqliteConnection:
                ...
                try:
                    with open(self.sql_delete_script, "r") as sqlite_file:
                        try:
                            sql_script = sqlite_file.read()
                            # print(sql_script)
                        except Exception as error:
                            print(f"Error while deleting the SQL script: {error}")
                            return
                except Exception as error:
